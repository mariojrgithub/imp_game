// source: https://github.com/ckknight/random-js
...
(function (root) {
  ...
  var stringRepeat =
    typeof String.prototype.repeat === "function" && "x".repeat(3) === "xxx"
      ? function (x, y) {
          return x.repeat(y);
        }
  ...
  function Random(engine) {
    ...
    if (engine == null) {
      ...
      throw new TypeError(
        "Expected engine to be a function, got " + typeof engine
      );
    }
    this.engine = engine;
  }
    ...
    mt19937: (function (Int32Array) {
      ...
      function refreshData(data) {
        ...
        for (; (k | 0) < 227; k = (k + 1) | 0) {
          ...
          data[k] =
            data[(k + 397) | 0] ^ (tmp >>> 1) ^ (tmp & 0x1 ? 0x9908b0df : 0);
        }

        for (; (k | 0) < 623; k = (k + 1) | 0) {
          tmp = (data[k] & 0x80000000) | (data[(k + 1) | 0] & 0x7fffffff);
          data[k] =
            data[(k - 227) | 0] ^ (tmp >>> 1) ^ (tmp & 0x1 ? 0x9908b0df : 0);
        }
      ...
      function seedWithArray(data, source) {
        ...
        for (k = 623; (k | 0) > 0; --k) {
          data[i] = previous =
            ((data[i] ^ imul(previous ^ (previous >>> 30), 0x5d588b65)) - i) |
            0;
          i = (i + 1) | 0;
          if ((i | 0) > 623) {
            data[0] = data[623];
  ...
  Random.integer = (function () {
    ...
    function upscaleToLoopCheckedRange(extendedRange) {
      var maximum =
        extendedRange * Math.floor(0x20000000000000 / extendedRange);
      return function (engine) {
        var ret = 0;
        do {
          var high = engine() & 0x1fffff;
    ...
    return function (min, max) {
      ...
      if (min < -0x20000000000000 || !isFinite(min)) {
        throw new RangeError(
          "Expected min to be at least " + -0x20000000000000
        );
    ...
    function probability(percentage) {
      if (percentage <= 0) {
        ...
        if (scaled % 1 === 0) {
          ...
          return lessThan(
            Random.uint53,
            Math.round(percentage * 0x20000000000000)
          );
        }
      }
    }
